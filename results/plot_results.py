...
#         ax[0, 0].bar(i + 0.75*j/len(methods) + 0.25/2, qp_solve_time.mean(), width=0.3, color= adjust_lightness(col, 0.5), linewidth=3, ec='k', label='qp')
#         ax[0, 0].bar(i + 0.75*j/len(methods) + 0.25/2, cbf_solve_time.mean(), bottom=qp_solve_time.mean(), width=0.3, color=adjust_lightness(col, 0.75), linewidth=3, ec='k', label='cbf')
#         ax[0, 0].bar(i + 0.75*j/len(methods) + 0.25/2, prune_time.mean(), bottom=cbf_solve_time.mean() + qp_solve_time.mean(), width=0.3, color = adjust_lightness(col, 1.), linewidth=3, ec='k', label='prune')
